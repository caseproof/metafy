...
2. Add 'metas' table to your database
   - Create migration: <b>rails g metafy</b>
   - Migrate your database: <b>rake db:migrate</b>

3. Add metafied attributes to your model using the <b>metafy</b> class method (in this case we're putting it at the top of our "Person" model):
...
OR select & order using standard rails relations:
  >> num_johnsons = Person.includes( :m_last_name ).includes( :m_first_name ).where( :m_last_name => { :meta_value => 'Johnson' } ).order( 'm_first_name.meta_value' )
