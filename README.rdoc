...
OR select using the meta_where scope:
  >> num_johnsons = User.where( :email => 'jerry@example.com' ).meta_where( :last_name => 'Johnson' ).count

OR select & order using standard rails relations:
  >> num_johnsons = User.where( :m_last_name => { :meta_value => 'Johnson' } ).order( 'm_first_name.meta_value DESC' )
